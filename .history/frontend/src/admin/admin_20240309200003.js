...
//           <div>
//             <h4>Items:</h4>
//             {order.order_details.map((detail, index) => ( // Note the change here to access order_details correctly
//               <div key={index}> {/* Using index as key because item_id can be repeated in different orders */}
...
const Admin = () => {
  ...
  useEffect(() => {
    authenticateAndFetchOrders(); // Call this function on initial load without a month filter
  }, []);
  ...
  const authenticateAndFetchOrders = async (selectedMonth = '') => {
    ...
    if (password === "admin") {
      setIsAuthenticated(true);
      const fetchUrl = selectedMonth
        ? `http://localhost:5000/admin?month=${selectedMonth}`
  ...
  return (
    <div className={styles.adminContainer}>
      ...
      {orders.map((order) => (
        <div key={order.order_id} className={styles.orderItem}>
          {/* Order rendering code remains the same */}
        </div>
      ))}
    </div>
  );
